// Package endpointtests implements static tests for the API layer.
...
// staticMain validates we can get the main page.
func staticMain(t *testing.T, a *app.App, c *app.Context) {
	r := tests.NewRequest("GET", "/", nil)
	w := httptest.NewRecorder()
	a.ServeHTTP(w, r)

	t.Log("Given the need to test the main static page.")
	{
		if w.Code != 200 {
...
import (
	"bytes"
	"encoding/json"
	"gotraining/13-http/api/app"
	"gotraining/13-http/api/models"
	"gotraining/13-http/api/routes"
	"gotraining/13-http/api/tests"
	"net/http/httptest"
	"testing"
		"github.com/vallard/stickypipe-receiver/app"
